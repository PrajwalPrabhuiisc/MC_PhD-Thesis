...
NOISE_STD = {"reporting": 0.07, "communication": 0.05, "mental_model": 0.03}
LEARNING_RATE = 0.10
DETECTION_ACCURACY = {"Worker": 0.6, "Manager": 0.7, "Director": 0.80}
...
class OrgAgent(Agent):
    def __init__(self, unique_id, model, role, scenario_components, scenario_weights):
        ...
        self.action = None  # Track the action taken (ACT or ESCALATE)
    ...
    def communicate(self):
        ...
        if report is not None:
            ...
            self.action = "ACT" if self.sa.sa > action_threshold else "ESCALATE"
            self.execute_action(self.action)
        else:
            self.action = None  # No action taken if no report generated
...
class OrgModel(Model):
    def __init__(self, scenario):
        ...
        # Data collector with additional variables
        self.datacollector = DataCollector(
            agent_reporters={
                ...
                # New agent-level variables
                "Initial_SA": lambda a: a.initial_sa,
                "Weight_P": lambda a: a.scenario_weights.get("P", 0.0),
                "Weight_C": lambda a: a.scenario_weights.get("C", 0.0),
                "Weight_J": lambda a: a.scenario_weights.get("J", 0.0),
                "Comp_Competence_P": lambda a: a.component_competence["P"],
                "Comp_Competence_C": lambda a: a.component_competence["C"],
                "Comp_Competence_J": lambda a: a.component_competence["J"],
                "Detection_Accuracy": lambda a: a.detection_accuracy,
                "Comm_Effectiveness": lambda a: a.communication.communication_effectiveness,
                "Successful_Comms": lambda a: a.communication.successful_communications,
                "Message_History_Count": lambda a: len(a.communication.message_history),
                "Messages_Received": lambda a: len(a.communication.messages),
                "Convergence_Rate": lambda a: a.mental_model.convergence_rate,
                "Scenario_Learning_Mod": lambda a: a.scenario_learning_modifier,
                "Previous_SA": lambda a: a.sa.previous_sa,
                "Strength_P": lambda a: a.sa.component_strengths.get("P", 0.0),
                "Strength_C": lambda a: a.sa.component_strengths.get("C", 0.0),
                "Strength_J": lambda a: a.sa.component_strengths.get("J", 0.0),
                "Trust_Partners": lambda a: len(a.trust),
                "Avg_Trust_Per_Partner": lambda a: np.mean(list(a.trust.values())) if a.trust else 0.0,
            ...
            model_reporters={
                ...
                # New model-level variables
                "Task_Complexity": lambda m: m.task_complexity,
                "Avg_Org_Structure": lambda m: np.mean(m.org_structure[m.org_structure > 0]) if np.any(m.org_structure > 0) else 0.0,
                "Avg_SA_Change": lambda m: m.avg_sa_change,
                "Step_Count": lambda m: m.step_count,
